...
def train_model(model, batch_gen, num_train_steps):
  ...
  utils.make_dir('/tmp/checkpoints')
  with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    ckpt = tf.train.get_checkpoint_state(os.path.dirname('/tmp/checkpoints/checkpoint'))
    # if that checkpoint exists, restore from checkpoint
    if ckpt and ckpt.model_checkpoint_path:
      saver.restore(sess, ckpt.model_checkpoint_path)

    total_loss = 0.0  # we use this to calculate late average loss in the last SKIP_STEP steps
    writer = tf.summary.FileWriter('/tmp/word2vec/improved_graph/lr' + str(LEARNING_RATE), sess.graph)
    ...
    for index in range(initial_step, initial_step + num_train_steps):
      ...
      if (index + 1) % SKIP_STEP == 0:
        ...
        total_loss = 0.0
        saver.save(sess, '/tmp/checkpoints/skip-gram', index)
